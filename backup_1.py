...
def ent_select(aug_all_ent):
    """Select augmentation entropies that have meaningful values (> 0.0001)"""
    aug_req_ent = []
...
def uncert_voting_improved(aug_output):
    ...
    for i in range(1, len(aug_output)): 
        prob = torch.sigmoid(aug_output[i] / temperature)
        aug_all_prob.append(prob)
        aug_all_ent.append(sigmoid_entropy(prob))
    
    no_aug_prob_nor = torch.sigmoid(aug_output[0] / temperature)
    
    margin = 0.05
    
    # Soft thresholding:
    no_aug_pseudo_label = torch.where(
        no_aug_prob_nor >= 0.5 + margin,
        torch.ones_like(no_aug_prob_nor), 
        torch.where(
            no_aug_prob_nor <= 0.5 - margin, 
            torch.zeros_like(no_aug_prob_nor),
            no_aug_prob_nor
        )
    )
    
    aug_prob_stack = torch.stack(aug_all_prob)
    aug_mean = aug_prob_stack.mean(dim=0)
    aug_var = aug_prob_stack.var(dim=0)  # Variance across augmentations
    
    reliability = 1.0 / (1.0 + aug_var + 1e-8)
    reliability = torch.clamp(reliability, 0.1, 1.0)
    
    ensemble_prob = reliability * aug_mean + (1 - reliability) * no_aug_prob_nor
    
    no_aug_ent = sigmoid_entropy(no_aug_prob_nor)
    no_aug_ent[torch.isnan(no_aug_ent)] = 0 
    
    aug_req_ent = ent_select(aug_all_ent)
    if len(aug_req_ent) > 0:
        aug_avg_ent = sum(aug_req_ent)/len(aug_req_ent)  # Average entropy across augmentations
    else:
        aug_avg_ent = torch.zeros_like(no_aug_ent)
    aug_avg_ent[torch.isnan(aug_avg_ent)] = 0  # Handle NaN
    
    eps = 1e-8  # Small epsilon
    
    # Normalize no-aug entropy
    no_aug_range = no_aug_ent.max() - no_aug_ent.min() + eps
    no_aug_ent_nor = (no_aug_ent - no_aug_ent.min()) / no_aug_range
    no_aug_ent_nor = torch.clamp(no_aug_ent_nor, 0, 1) 
    
    # Normalize aug entropy  
    aug_range = aug_avg_ent.max() - aug_avg_ent.min() + eps
    aug_avg_ent_nor = (aug_avg_ent - aug_avg_ent.min()) / aug_range
    aug_avg_ent_nor = torch.clamp(aug_avg_ent_nor, 0, 1)


    # Adaptive wiehgt
    agreement = 1.0 - torch.abs(no_aug_prob_nor - ensemble_prob)  # Agreement between no-aug và ensemble
    adaptive_weight = 0.5 + 0.3 * agreement
    
    weighted_ent = adaptive_weight * no_aug_ent_nor + (1 - adaptive_weight) * aug_avg_ent_nor
    
    uncertainty_thresh = 0.4 
    weighted_ent_thresh = weighted_ent.clone()
    weighted_ent_thresh[weighted_ent_thresh > uncertainty_thresh] = 1
    weighted_ent_thresh[weighted_ent_thresh <= uncertainty_thresh] = 0
    
    prob_min = 0.25  # Lower threshold để capture more uncertain cases
    prob_max = 0.75  # Upper threshold
    
    prob_uncert = (no_aug_prob_nor > prob_min) & (no_aug_prob_nor < prob_max)
    
    agreement_uncert = torch.abs(no_aug_prob_nor - ensemble_prob) > 0.1
    
    combined_uncert = (prob_uncert | agreement_uncert).float()
    
    pseudo_uncert = combined_uncert * weighted_ent_thresh.float()
    
    confidence_weight = 1.0 - weighted_ent  # High confidence = low entropy
    
    final_pseudo = (confidence_weight * no_aug_pseudo_label.float() + 
                   (1 - confidence_weight) * ensemble_prob +
                   0.3 * pseudo_uncert)
    
    final_pseudo = torch.clamp(final_pseudo, 0, 1)
    
    return final_pseudo.unsqueeze(0).float()

def calculate_uncertainty_weights(aug_output):
    temperature = 1.3  # Consistent với uncertainty voting
    
    # Get probabilities from ensemble output with temperature scaling
    ensemble_probs = torch.stack([torch.sigmoid(out / temperature) for out in aug_output], dim=0)
    
    # Calculate the standard deviation of probabilities for each pixel as uncertainty measure
    uncertainty = ensemble_probs.std(dim=0)
    
    no_aug_prob = ensemble_probs[0]
    aug_mean_prob = ensemble_probs[1:].mean(dim=0)
    consistency = 1.0 - torch.abs(no_aug_prob - aug_mean_prob)  # High consistency = low difference
    
    base_confidence = 1.0 - uncertainty
    consistency_boost = consistency * 0.3
    
    weights = base_confidence + consistency_boost
    
    # Normalize weights to be in the [0.1, 1.0] range (avoid zero weights)
    weights = (weights - weights.min()) / (weights.max() - weights.min() + 1e-6)
    weights = weights * 0.9 + 0.1  # Scale to [0.1, 1.0]
...
def sfuda_target(config, train_loader, pseduo_model, msrc_model, criterion, optimizer, epoch):
    ...
    for input, target, path in train_loader:
        ...
        with torch.no_grad():
            # Get predictions from pseudo model
            aug_output = pseduo_model(aug_input.cuda())
            
            # CẢI TIẾN 1: Sử dụng improved uncertainty voting
            # IMPROVEMENT 1: Use improved uncertainty voting
            pseudo_labels = uncert_voting_improved(aug_output.detach())
            
            # CẢI TIẾN 2: Tính enhanced confidence weights
            # IMPROVEMENT 2: Calculate enhanced confidence weights
            uncertainty_weights = calculate_uncertainty_weights(aug_output.detach())
        
        # Forward pass với student model
        optimizer.zero_grad()
        output = msrc_model(aug_input.cuda())
        
        # CẢI TIẾN 3: Apply confidence weights cho segmentation loss
        # IMPROVEMENT 3: Apply confidence weights to segmentation loss
        ...
        # CẢI TIẾN 4: Dynamic entropy loss weighting dựa trên epoch
        # IMPROVEMENT 4: Dynamic entropy loss weighting based on epoch
        # Trọng số giảm dần từ 1.0 về 0.1 khi tiến gần đến cuối stage (không về 0 hoàn toàn)
        # Weight gradually decreases from 1.0 to 0.1 as approaching end of stage (not completely to 0)
        ent_weight = 0.9 * (1.0 - (epoch / config['stage1'])) + 0.1  # From 1.0 to 0.1
        ent_loss = sigmoid_entropy_loss(torch.sigmoid(output))
        
        # CẢI TIẾN 5: Add consistency regularization giữa different augmentations
        # IMPROVEMENT 5: Add consistency regularization between different augmentations
        if len(aug_output) > 1:
            # Consistency loss giữa predictions của các augmentations
            consistency_loss_aug = 0
            for i in range(len(output)-1):
                for j in range(i+1, len(output)):
                    consistency_loss_aug += F.mse_loss(
                        torch.sigmoid(output[i]), 
                        torch.sigmoid(output[j])
                    )
            consistency_loss_aug = consistency_loss_aug / (len(output) * (len(output)-1) / 2)
            consistency_weight = 0.1  # Small weight cho consistency
        else:
            consistency_loss_aug = 0
            consistency_weight = 0
        
        # Kết hợp tất cả losses với improved weights
        # Combine all losses with improved weights
        loss = seg_loss + ent_weight * ent_loss + consistency_weight * consistency_loss_aug
