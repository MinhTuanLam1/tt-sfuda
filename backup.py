...
def build_strong_augmentation(img):
    """
    Sử dụng các phép biến đổi hình học và cường độ mạnh mẽ hơn
    để tăng cường độ bền bỉ của mô hình.
    """
    strong_aug = T.Compose([
        # Biến đổi ngẫu nhiên mạnh về màu sắc và độ sáng
        T.RandomApply([T.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=0.6),
        
        # Biến đổi hình học cơ bản nhưng hiệu quả
        T.RandomApply([
            T.RandomAffine(degrees=(-15, 15), translate=(0.1, 0.1), scale=(0.9, 1.1)),
        ], p=0.8),

        # Lật ngẫu nhiên, rất hiệu quả cho việc tăng cường dữ liệu
        T.RandomHorizontalFlip(p=0.5),
        T.RandomVerticalFlip(p=0.5),

        # Biến đổi sang ảnh xám để mô hình không phụ thuộc màu sắc
